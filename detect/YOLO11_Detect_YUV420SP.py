...
class VisionDetector:
    ...
    def __init__(self, model_path: str, conf_thresh: float = 0.25, iou_thresh: float = 0.45,
        ...
        :param min_aspect_ratio: 最小长宽比（宽/高）
        :param max_aspect_ratio: 最大长宽比（宽/高）
        """
        ...
        logger.info(f"视觉检测器初始化完成 - 长宽比筛选: {min_aspect_ratio:.1f}-{max_aspect_ratio:.1f}")
    
    def filter_by_aspect_ratio(self, detections: list) -> list:
        """
        根据长宽比筛选检测结果
        ...
        for detection in detections:
            try:
                ...
                if self.min_aspect_ratio <= aspect_ratio <= self.max_aspect_ratio:
                    ...
                    logger.debug(f"长宽比筛选: 移除检测框 长宽比={aspect_ratio:.2f} (要求:{self.min_aspect_ratio:.1f}-{self.max_aspect_ratio:.1f})")
                    
            except (KeyError, IndexError, ZeroDivisionError) as e:
                logger.warning(f"长宽比筛选时出错: {e}")
                continue
        
        filtered_count = original_count - len(filtered_detections)
        if filtered_count > 0:
            self.filtered_count += filtered_count
            logger.debug(f"长宽比筛选: {original_count} -> {len(filtered_detections)} (移除{filtered_count}个)")
    ...
    def detect(self, frame: np.ndarray) -> dict:
        ...
        try:
            ...
            # 新增：应用长宽比筛选
            filtered_detections = self.filter_by_aspect_ratio(detections)
            
            result = {
                'success': True,
                'detections': filtered_detections,
                'count': len(filtered_detections),
                'original_count': len(detections),  # 新增：记录原始检测数量
                'filtered_count': len(detections) - len(filtered_detections)  # 新增：记录被筛选的数量
            }
    ...
    def get_best_detection_from_result(self, result: dict, target_class: str = "square") -> dict:
        ...
        return {
            ...
            'aspect_ratio': best_detection['size'][0] / max(best_detection['size'][1], 1)  # 新增：记录长宽比
        }
    
    def get_best_detection(self, frame: np.ndarray, target_class: str = "square") -> dict:
        """
        获取最佳检测结果（置信度最高的目标）- 修复缺失的方法
        :param frame: 输入图像
...
if __name__ == "__main__":
    ...
    try:
        detector = VisionDetector("/root/square/detect/3.0.bin")
        ...
        print("按 'r' 重置检测缓存")
        print("按 's' 显示详细统计")
        print("按 'd' 切换调试模式（显示白色四边形检测过程）")  
        print("按 'b' 显示二值化过程（需要配合vision_pipeline使用）")  # 新增说明
        
        debug_mode = False  # 新增调试模式标志
        ...
        while True:
            ...
            try:
                ...
                if detection_result['found']:
                    ...
                    center_x, center_y = detection_result['center']
                    confidence = detection_result['confidence']
                    size = detection_result.get('size', [0, 0])
                    aspect_ratio = detection_result.get('aspect_ratio', 0)
                    
                    if fps_counter % 10 == 0:  # 每10帧输出一次
                        if debug_mode:
                            print(f"\r帧{fps_counter}: 检测到方块 ({center_x}, {center_y}) 置信度:{confidence:.2f} 尺寸:{size[0]}x{size[1]} 长宽比:{aspect_ratio:.2f} 耗时:{detection_time*1000:.1f}ms", end="", flush=True)
                        else:
            ...
            if key == ord('q'):
                ...
                if fps_counter > 0:
                    ...
                    # 获取长宽比筛选统计
                    filter_stats = detector.get_filter_statistics()
                    ...
                    print(f"长宽比筛选: 范围{filter_stats['min_aspect_ratio']:.1f}-{filter_stats['max_aspect_ratio']:.1f}, 已筛选{filter_stats['total_filtered']}个")
                    print("="*60)
